...
def PI_capture(cam: "cv.Camera") -> bytes:
    ...
    return cast(bytes, (pybase64.b64encode(frame)))             # Then convert to base64 format


def WIN_capture(cam: Any) -> bytes:
    _, frame = cam.read()                                       # Obtain the frame.
    _, frame = cv.imencode('.jpg', frame, ENCODEQ_65)           # Encode from image to binary buffer
    return cast(bytes, (pybase64.b64encode(frame)))             # Then vonvert to base64 format
