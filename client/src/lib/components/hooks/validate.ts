...
export const validate = (msg: any, topic: string) => {
    switch(topic) {
        case "vitals-topic": {
            ...
            assert(Math.abs(vital.airHumidity - expected_vital.airHumidity) < EPSILON)
            assert(Math.abs(vital.greenGrowth - expected_vital.greenGrowth) < EPSILON)
            assert(Math.abs(vital.light - expected_vital.light) < EPSILON)
            assert(Math.abs(vital.soilMoisture - expected_vital.soilMoisture) < EPSILON)
            assert(Math.abs(vital.temperature - expected_vital.temperature) < EPSILON)
